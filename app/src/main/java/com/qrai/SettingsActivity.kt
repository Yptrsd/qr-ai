...
class SettingsActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        titleBar.addView(TextView(this).apply {
            text = "←"; textSize = 22f; setPadding(0, dp(8), dp(16), dp(8))
            setOnClickListener { save(); finish() }
        })
        titleBar.addView(TextView(this).apply {
            text = getString(R.string.settings_title); textSize = 20f; typeface = Typeface.DEFAULT_BOLD
        ...
        val apiListScroll = ScrollView(this).apply {
        ...
        root.addView(Button(this).apply {
            ...
            textSize = 14f; setTextColor(0xFF1A73E8.toInt())
            setBackgroundColor(Color.TRANSPARENT); elevation = 0f
            setOnClickListener { showAddApiDialog() }
        })

        // ── System Prompt ──
        root.addView(label("System Prompt"))
        systemInput = EditText(this).apply {
            hint = getString(R.string.default_system_prompt)
            setText(prefs().getString("system", "") ?: "")
            textSize = 15f; minLines = 2; maxLines = 5
            setTextColor(0xFF303030.toInt()); setHintTextColor(0xFFBBBBBB.toInt())
        ...
        // ── 保存 ──
        root.addView(Button(this).apply {
            text = getString(R.string.btn_save); textSize = 16f
            setTextColor(0xFFFFFFFF.toInt()); background = getDrawable(R.drawable.btn_bg); elevation = 0f
            val lp = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(48))
            lp.topMargin = dp(28); layoutParams = lp
            setOnClickListener {
                save()
    ...
    private fun refreshApiList() {
        ...
        if (apis.isEmpty()) {
            apiListContainer.addView(TextView(this).apply {
                text = getString(R.string.api_list_empty); textSize = 13f
                setTextColor(0xFFBBBBBB.toInt()); setPadding(0, dp(8), 0, dp(8))
            })
            return
        }
        apis.forEachIndexed { i, api ->
            ...
            row.addView(TextView(this).apply {
                text = if (active) "● " else "○ "; textSize = 16f
                setTextColor(if (active) 0xFF1A73E8.toInt() else 0xFF999999.toInt())
                setPadding(0, 0, dp(8), 0)
                setOnClickListener { activeIndex = i; refreshApiList() }
            })
            // 名称 → 点击编辑（也支持直接选中后再编辑）
            row.addView(TextView(this).apply {
                text = name; textSize = 15f; setTextColor(0xFF303030.toInt())
                val lp = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f); layoutParams = lp
                setOnClickListener { showAddApiDialog(i) }
            })
            row.addView(TextView(this).apply {
                text = api.optString("model", ""); textSize = 12f; setTextColor(0xFF999999.toInt())
            })
            row.addView(TextView(this).apply {
                text = when (api.optString("endpoint", "chat")) {
                    "responses" -> " [R]"
                    "auto" -> " [A]"
                    else -> " [C]"
                }
                textSize = 10f; setTextColor(0xFF1A73E8.toInt())
            })
            // 长按删除（整行均可触发）
            row.setOnLongClickListener {
                AlertDialog.Builder(this).setTitle(getString(R.string.dialog_delete_title))
    ...
    private fun showAddApiDialog(editIndex: Int = -1) {
        ...
        val contentScroll = ScrollView(ctx).apply {
        ...
        fun field(h: String, v: String, secret: Boolean = false, multi: Boolean = false) = EditText(ctx).apply {
            hint = h; setText(v); textSize = 14f; isSingleLine = !multi
            setTextColor(0xFF303030.toInt()); setHintTextColor(0xFF999999.toInt())
            setBackgroundColor(0xFFF0F0F0.toInt()); setPadding(dp(16), dp(12), dp(16), dp(12))
            inputType = when {
        ...
        presetSpinner.adapter = ArrayAdapter(ctx, android.R.layout.simple_spinner_item, presets).apply {
        ...
        val urlEt = field(getString(R.string.label_base_url), editApi?.optString("url", "") ?: "https://api.deepseek.com")
        ...
        val endpointSpinner = Spinner(ctx).apply {
            layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(44))
            background = getDrawable(R.drawable.spinner_bg)
        }
        endpointSpinner.adapter = ArrayAdapter(ctx, android.R.layout.simple_spinner_item, endpoints).apply {
            setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        }
        ...
        // ── 联网搜索模板 ──
        val webJson = field(getString(R.string.hint_web_search),
        ...
        // ── 模型 Spinner + 获取按钮 ──
        val modelRow = LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL
        }
        val modelSpinner = Spinner(ctx).apply {
            layoutParams = LinearLayout.LayoutParams(0, dp(44), 1f)
            background = getDrawable(R.drawable.spinner_bg)
            // 初始隐藏，点"获取"成功后才显示（避免与手输框同时出现）
            visibility = android.view.View.GONE
        }
        val modelLoading = ProgressBar(ctx).apply {
            layoutParams = LinearLayout.LayoutParams(dp(36), dp(36))
            visibility = android.view.View.GONE
        }
        val fetchBtn = TextView(ctx).apply {
            text = getString(R.string.btn_fetch); textSize = 13f; setTextColor(0xFF1A73E8.toInt())
        ...
        layout.addView(label(getString(R.string.label_name))); layout.addView(nameEt)
        layout.addView(label(getString(R.string.label_api_key))); layout.addView(keyEt)
        layout.addView(label(getString(R.string.label_base_url))); layout.addView(urlEt)
        layout.addView(label(getString(R.string.label_model))); layout.addView(modelRow)

        // 手动输入兜底：编辑模式显示（预填当前模型），新增模式隐藏（点获取失败才显示）
        val manualModel = field(getString(R.string.hint_manual_model),
            if (isEdit) editApi?.optString("model", "") ?: "" else "").apply {
        ...
        fun applyPreset(index: Int) {
            ...
            var web = ""
            when (index) {
                0 -> { name = getString(R.string.preset_deepseek); url = "https://api.deepseek.com"; endpointIdx = 1
                    web = """{"tools":[{"type":"web_search"}],"tool_choice":"auto"}""" }
                1 -> { name = getString(R.string.preset_openai); url = "https://api.openai.com"; endpointIdx = 1
                    web = """{"tools":[{"type":"web_search"}],"tool_choice":"auto"}""" }
                2 -> { name = getString(R.string.preset_mimo); url = "https://api.xiaomimimo.com/v1"; endpointIdx = 0
                    web = """{"tools":[{"type":"web_search","max_keyword":3,"force_search":true,"limit":1}],"tool_choice":"auto"}""" }
                3 -> { name = getString(R.string.preset_anthropic); url = "https://api.anthropic.com"; endpointIdx = 0
                    web = """{"tools":[{"type":"web_search_20250305"}]}""" }
                4 -> { name = getString(R.string.preset_kimi); url = "https://api.moonshot.cn/v1"; endpointIdx = 0
                    web = """{"tools":[{"type":"web_search"}],"tool_choice":"auto"}""" }
                5 -> { name = getString(R.string.preset_glm); url = "https://open.bigmodel.cn/api/paas/v4"; endpointIdx = 0
                    web = """{"tools":[{"type":"web_search"}],"tool_choice":"auto"}""" }
                6 -> { name = getString(R.string.preset_grok); url = "https://api.x.ai"; endpointIdx = 1
                    web = """{"tools":[{"type":"web_search_preview"}],"tool_choice":"auto"}""" }
                7 -> { name = getString(R.string.preset_gemini); url = "https://generativelanguage.googleapis.com/v1beta/openai"; endpointIdx = 0
                    web = """{"tools":[{"type":"googleSearch"}],"tool_choice":"auto"}""" }
            }
            nameEt.setText(name)
            urlEt.setText(url)
        ...
        presetSpinner.onItemSelectedListener = object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: android.view.View?, position: Int, id: Long) {
                // 只有位置变化才应用（用户手动选择），初始/重复回调不覆盖预填值
        ...
        if (isEdit) {
            ...
            endpointSpinner.setSelection(when (editApi?.optString("endpoint", "chat")) {
                "responses" -> 1
                "auto" -> 2
                else -> 0
            })
        ...
        val dialog = AlertDialog.Builder(ctx)
            .setTitle(if (isEdit) getString(R.string.dialog_edit_title) else getString(R.string.dialog_add_title))
            .setView(contentScroll)
            .setPositiveButton(if (isEdit) getString(R.string.btn_save_apply) else getString(R.string.btn_add), null) // 先设 null，下面用 setOnShow 绑定
            .setNegativeButton(getString(R.string.btn_cancel), null)
            .create()

        // 在对话框显示后绑定正面按钮逻辑（show 之后按钮才存在，且此写法每次打开可靠生效）
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                // 优先取手输框（编辑模式固定有值），其次 Spinner，最后回退原值
                val manual = manualModel.text.toString().trim()
                val spinnerVal = if (modelSpinner.visibility == android.view.View.VISIBLE && modelList.isNotEmpty())
                    modelSpinner.selectedItem?.toString() ?: "" else ""
                val selectedModel = manual.ifEmpty { spinnerVal }
                    .ifEmpty { if (isEdit) (editApi?.optString("model", "") ?: "") else "" }
                ...
                if (web.isNotEmpty()) {
                    try { JSONObject(web) } catch (e: Exception) {
                        Toast.makeText(ctx, getString(R.string.toast_web_json_error), Toast.LENGTH_SHORT).show()
                        return@setOnClickListener
                    }
                }
                val endpoint = when (endpointSpinner.selectedItemPosition) {
                    1 -> "responses"
                    2 -> "auto"
                    else -> "chat"
                }
                if (isEdit) {
                    editApi!!.put("name", nameEt.text.toString().trim().ifEmpty { "API ${editIndex + 1}" })
                    editApi.put("key", keyEt.text.toString().trim())
                    editApi.put("url", urlEt.text.toString().trim().ifEmpty { "https://api.deepseek.com" })
                    editApi.put("model", selectedModel)
                    editApi.put("endpoint", endpoint)
                    editApi.put("webSearch", web)
                } else {
                    val api = JSONObject().apply {
                        put("name", nameEt.text.toString().trim().ifEmpty { "API ${apis.size + 1}" })
                        put("key", keyEt.text.toString().trim())
                        put("url", urlEt.text.toString().trim().ifEmpty { "https://api.deepseek.com" })
                ...
                if (DEBUG) {
                    try {
                        AlertDialog.Builder(ctx)
                            ...
                            .setMessage(getString(R.string.debug_save_btn_msg,
                                isEdit, editIndex, selectedModel, editApi == null, editApi?.toString() ?: "null"))
                            .setPositiveButton(getString(R.string.btn_close), null)
                            .show()
                    } catch (_: Exception) {}
                }

                save()  // 立即持久化，确保编辑/添加可靠保存
                Toast.makeText(ctx, getString(R.string.toast_saved_config), Toast.LENGTH_SHORT).show()
                dialog.dismiss()
            }
        }
        ...
        // 获取模型列表（modelList 已在上方声明，这里只用 doFetch）
        fun doFetch() {
            val k = keyEt.text.toString().trim()
            val u = urlEt.text.toString().trim().ifEmpty { "https://api.deepseek.com" }
            if (k.isEmpty()) {
                Toast.makeText(ctx, getString(R.string.toast_need_api_key), Toast.LENGTH_SHORT).show()
                return
            }
                ...
                try {
                    ...
                    if (conn.responseCode == 200) {
                        ...
                        for (i in 0 until data.length()) {
                            ...
                            // 兜底：剔除旧版推理模型名（V4 时代思考已由 thinking 参数控制，模型名如 deepseek-v4-flash 不受影响）
                            if (id.isEmpty() || id.contains("reason", ignoreCase = true)) continue
                            modelList.add(id)
                        }
                        modelList.sort()
                        ui.post {
                            ...
                            if (modelList.isEmpty()) {
                                Toast.makeText(ctx, getString(R.string.toast_no_models), Toast.LENGTH_SHORT).show()
                                fetchBtn.visibility = android.view.View.VISIBLE
                            } else {
                                val adapter = ArrayAdapter(ctx, android.R.layout.simple_spinner_item, modelList)
                                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                                modelSpinner.adapter = adapter
                                modelSpinner.visibility = android.view.View.VISIBLE
                                // 二选一：有列表时隐藏手输框
                                manualModel.visibility = android.view.View.GONE
                                Toast.makeText(ctx, getString(R.string.toast_models_fetched, modelList.size), Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        ui.post {
                            ...
                            Toast.makeText(ctx, getString(R.string.toast_fetch_failed, conn.responseCode), Toast.LENGTH_SHORT).show()
                        }
                    }
                } catch (e: Exception) {
                    ui.post {
                        ...
                        fetchBtn.visibility = android.view.View.VISIBLE
                        manualModel.visibility = android.view.View.VISIBLE
                        Toast.makeText(ctx, getString(R.string.toast_network_error, e.message ?: ""), Toast.LENGTH_SHORT).show()
                    }
                } finally {
                    conn?.disconnect()
                }
        ...
        if (DEBUG) {
            try {
                AlertDialog.Builder(this)
                    ...
                    .setMessage(getString(R.string.debug_save_msg, apis.size, activeIndex, arr.toString(), arr.toString().length))
                    .setPositiveButton(getString(R.string.btn_close), null)
                    .show()
            } catch (_: Exception) {}
        }
    ...
    private fun label(s: String) = TextView(this).apply {
        text = s; textSize = 13f; setTextColor(0xFF888888.toInt()); setPadding(0, dp(20), 0, dp(6))
    }
